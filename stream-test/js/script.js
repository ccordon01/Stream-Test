...
var config = {
  ...
  network: {
    retryHttpRequest: function (type, response, retry) {
      console.error(`${type} - ${JSON.stringify(response)} - ${retry}`)
      if (retry == 3 || retry < 1) {
        if(type == 'manifest/dash') {
          response.request.url = 'http://p-cdn1-103-cg14-linear-cbd46b77.movetv.com/clipslist/1290/20200205T200000Z/20200205T210000Z.mpd';
  ...
  if (drm !== 'none') {
    ...
    tempSource.drm[drm] = {
      ...
      prepareMessage: (keyMessage) => {
        let message = Array.apply(null, new Uint8Array(keyMessage.message));
        return JSON.stringify({ message, "env": "production", "user_id": "54f5b9ce-d622-11e7-8f74-0eab56b32b92", "channel_id": "c763c4c3001f4a04970bca35000c0ded" });
      }
    };
  }
...
function setPlayerEvents(player) {
  player.on(bitmovin.player.PlayerEvent.VideoAdaptation, function (data) {
    console.log(`[BitmovinVideoRenderer::onVideoAdaptation] - event data :${JSON.stringify(data)}`);
    // updateCharts(player);
  });

  player.on(bitmovin.player.PlayerEvent.SegmentRequestFinished, function (data) {
    //log("On SegmentRequestFinished: " + JSON.stringify(data));
    // updateCharts(player);
  });
